...
// returns std::variant because it might have to return more than one possible type later
std::variant<std::string> ShowMainMenuBar(ap::Window* win, ap::Renderer& renderer, SaveScreenShotData& saveArea)
{
	std::string filepath;
	if (ImGui::BeginMainMenuBar())
	{
		if (ImGui::BeginMenu("File"))
		{
			...
			{
				filepath = SaveFile("Text Files\0*.txt\0Any File\0*.*\0", win);
			...
			{
				filepath = OpenFile("Text Files\0*.txt\0Any File\0*.*\0", win);
				if (!filepath.empty())
...
{	
	...
	{
		std::string filepath = OpenFile("Text Files\0*.txt\0Any File\0*.*\0", win);
		if (!filepath.empty())
		{
...
{	
	...
	std::vector<std::vector<Drawable*>::iterator> erasingLines;
	Line currentBuffer;
	bool drawing = false;
	ap::Clock drawClock; // to limit the amount of verticies the renderer has to draww
	...
	enum ConfigSlot { QUAD, CIRCLE, TRIANGLE, IMAGE };
	std::array<PreviewConfiguration, 5> configurations;	
	std::vector<std::future<void>> futures;
	SaveScreenShotData saveAreaData;
	while (window.isOpen())
	{		
		ImGui_ImplOpenGL3_NewFrame();
		ImGui_ImplGlfw_NewFrame();
		ImGui::NewFrame();		
		std::variant<std::string> vardata = ShowMainMenuBar(&window, renderer, saveAreaData);
		...
		glClearColor(renderClearColor.x, renderClearColor.y, renderClearColor.z, renderClearColor.w);
		{
			...
			if (ImGui::CollapsingHeader("Make A Quad!"))
			{
				...
				else if (qpc.previewDrawable != nullptr 
					&& qpc.showPreview)
				{
			...
			if (ImGui::CollapsingHeader("Make A Circle!"))
			{
				...
				else if (cpc.previewDrawable != nullptr
					&& cpc.showPreview)
				{
		...
		if (ap::Window::isMouseButtonPressed(ap::AP_MOUSE_BUTTON_LEFT) && !ImGui::IsAnyItemActive()
			&& drawClock.GetElapsedTimeAsMilliseconds() >= 10.0f)
		{					
